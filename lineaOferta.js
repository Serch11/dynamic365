...
function changeProduct(executionContext) {
    ...
    if (producid.getValue()) {
        if (producid.getValue()[0].id) {
            ...
            Xrm.WebApi.retrieveMultipleRecords('ap_exhibit', "?$select=ap_exhibitid,createdon,ap_porcentaje,_ap_producto_value&$filter=(statecode eq 0 and _ap_producto_value eq '" + id + "')&$orderby=createdon asc").then(
