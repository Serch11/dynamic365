...
function mensajeOportunidad(executionContext) {
    ...
    function viewMessage() {
        if (formContext.getControl("header_process_ap_ofertaaprobada").getAttribute().getValue() && formContext.getAttribute("statuscode").getValue() != 3) {
            formContext.ui.setFormNotification('DONT FORGET TO CLOSE THE OPPORTUNITY AS WON IN THE BUTTON ON THE TOP', 'WARNING', 'notification_unique_id');
                                            ...
                                            if (results.value.length > 0) {
                                                results.value.forEach((value, index) => {
                                                    ...
                                                    if (ap_codigodeasignaciondelprospectoid) {
                                                        ...
                                                        console.log(entity);
                                                        console.log("codigo de asignacion del prospecto " + ap_codigodeasignaciondelprospectoid);
                                                        let req = new XMLHttpRequest();
